...
pub struct RegisterFile {
    ...
    enabled: bool,
    write_buffer: (RegisterIndex, Immediate),
...
impl RegisterFile {
    ...
    pub(crate) fn clock(&mut self) {
        if self.enabled {
            let (index, value): (RegisterIndex, Immediate) = self.write_buffer;
            if self.is_valid_index(index) {
